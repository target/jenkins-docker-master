...
### Modify the image to company specific settings
  ...
  ```dockerfile
  FROM target/jenkins-docker-master:2.107.3-1
...
### Create the Docker Swarm service
  ...
  target/jenkins-docker-master:2.107.3-1
  ```
...
### `jenkins-docker-master:2.107.3-1`

This is the de facto image. It is based off of `jenkins/jenkins:2.107.3` and includes a few modifications. It was created with the intention to be extended by using groovy scripts to setup ACLs using various plugins and as such, we have provided example groovy scripts [here](https://github.com/target/jenkins-docker-master/blob/master/examples).

### `jenkins-docker-master:debug-2.107.3-1`

This image is based off of the `jenkins-docker-master:2.107.3-1` image. The noticeable difference is in relation to increasing log verbosity for troubleshooting.
